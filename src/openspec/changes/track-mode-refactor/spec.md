...
### Requirement: Reset track timing state
The system MUST reset track timing values when a reset action is performed.

#### Scenario: User resets timing
**When** reset is triggered
**Then** current lap, previous lap, best lap, and delta return to their initial state
**And** the display shows a neutral placeholder for missing valuess